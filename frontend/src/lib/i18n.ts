export type Lang = 'uz' | 'ru'

export const UI_TEXT = {
  uz: {
    brandName: 'BOZOR TAHLILI',
    sidebarTitle: 'Kredit mahsulotlari',
    sidebarFooter: "SQB · Strategiya bo'limi",
    poweredByLabel: 'SQB AI tomonidan',
    adminSectionLabel: 'Boshqaruv',
    adminUsersNavLabel: 'Foydalanuvchilar',
    pageSubtitle: 'Raqobatchi banklar shartlarining joriy holati',
    tableRank: '#',
    tableBank: 'Bank',
    tableProduct: 'Mahsulot',
    tableRate: 'Stavka',
    tableTerm: 'Muddat',
    tableDownPayment: "Boshlang'ich badal",
    tableGracePeriod: 'Imtiyozli davr',
    tableAmount: 'Kredit miqdori',
    tableSpecialTerms: 'Maxsus shartlari',
    tablePaymentMethod: "To'lov usuli",
    yes: 'Bor',
    no: "Yo'q",
    dash: '-',
    defaultPaymentMethod: 'Annuitet, Differensial',
    emptyState: "Bu toifa uchun hozircha ma'lumot yo'q - banklar sahifalari navbatda kuzatilmoqda.",
    houseFlag: 'Biz',
    pulseEyebrow: "Bozor pulsi · Eng zo'r taklif",
    pulseUpdated: 'Yangilangan',
    pulseRate: 'Stavka',
    pulseTerm: 'Muddat',
    pulseAmount: 'Kredit miqdori',
    pulseCollateral: 'Garov',
    pulsePaymentMethod: "To'lov usuli",
    pulseAiBadge: 'AI',
    pulseAiLabel: 'AI tavsiyasi',
    pulseAiLoading: 'Tahlil qilinmoqda...',
    unavailableReasonUnavailable: 'Mahsulot mavjud emas',
    unavailableReasonPaused: "Vaqtincha to'xtatilgan",
    langToggleLabel: 'Til',
    langOptionUz: "O'zbekcha",
    langOptionRu: 'Ruscha',
    monthUnit: 'oy',
    loadingLabel: 'Yuklanmoqda',
    exportButton: "Excel'ga yuklash",
    exportCurrentPage: 'Joriy sahifani yuklash',
    exportAllCategories: 'Barcha kategoriyalarni yuklash',
    exportFailed: "Yuklab bo'lmadi. Birozdan so'ng qayta urinib ko'ring.",
    refreshButton: 'Ma\'lumotlarni yangilash',
    refreshConfirmTitle: 'Ma\'lumotlarni qayta yig\'ish',
    refreshConfirmBody:
      "Barcha banklarning ma'lumotlari qaytadan yig'iladi. Bu jarayon bir necha daqiqa vaqt oladi - yangi ma'lumotlar tayyor bo'lguncha jadvallar hozirgi holatida qoladi. Davom etasizmi?",
    refreshConfirmYes: 'Ha, boshlash',
    refreshConfirmCancel: 'Bekor qilish',
    refreshStarted:
      "Yangilash boshlandi. Barcha banklar qayta tekshirilmoqda - iltimos, taxminan 5 daqiqadan so'ng sahifani qayta yuklang.",
    refreshAlreadyRunning: 'Yangilash allaqachon ishlamoqda - iltimos, biroz kuting.',
    refreshFailed: "Yangilashni boshlab bo'lmadi. Birozdan so'ng qayta urinib ko'ring.",
    closeLabel: 'Yopish',
    logoutButton: 'Chiqish',
    authTitle: 'Tizimga kirish',
    authUsernameLabel: 'Login',
    authPasswordLabel: 'Parol',
    authSubmitButton: 'Kirish',
    authError: "Login yoki parol noto'g'ri",
    authRateLimited: "Juda ko'p urinish. Iltimos, bir necha daqiqadan so'ng qayta urinib ko'ring.",
    adminPanelTitle: 'Foydalanuvchilarni boshqarish',
    adminPanelSubtitle: "Tizimga kirish huquqi bo'lgan foydalanuvchilar ro'yxati",
    adminAddUserButton: "Yangi user qo'shish",
    adminUsernameLabel: 'Login',
    adminRoleLabel: 'Rol',
    adminColCreatedAt: "Qo'shilgan sana",
    adminEditButton: 'Tahrirlash',
    adminEditUserTitle: 'Userni tahrirlash',
    adminAddUserTitle: 'Yangi user',
    adminPasswordLabel: 'Parol',
    adminPasswordLabelOptional: 'Yangi parol (ixtiyoriy)',
    adminPasswordPlaceholder: "O'zgartirmaslik uchun bo'sh qoldiring",
    adminPasswordHint: 'Kamida 8 belgi',
    adminRoleUser: 'Foydalanuvchi',
    adminRoleAdmin: 'Administrator',
    adminSaveButton: 'Saqlash',
    adminUsernameTaken: 'Bu login band',
    adminSelfDemote: "O'z rolingizni o'zgartira olmaysiz",
    adminPasswordTooShort: 'Parol kamida 8 belgidan iborat bo\'lishi kerak',
    adminSaveFailed: "Saqlab bo'lmadi. Qayta urinib ko'ring.",
    adminLoadFailed: "Userlarni yuklab bo'lmadi",
    scrapeStatusTitle: 'Scraperlar holati',
    scrapeStatusSubtitle: "Har bir bank uchun oxirgi ma'lumot yig'ish urinishi",
    scrapeStatusLoadFailed: "Scraper holatini yuklab bo'lmadi",
    scrapeColStatus: 'Holat',
    scrapeColLastRun: 'Oxirgi urinish',
    scrapeColProductsFound: 'Topilgan mahsulot',
    scrapeStatusSuccess: 'Muvaffaqiyatli',
    scrapeStatusFailed: 'Xato',
    scrapeStatusRunning: 'Ishlamoqda',
    scrapeStatusNoProducts: 'Diqqat talab (0 mahsulot)',
    scrapeStatusNeverRun: 'Hech qachon ishlamagan',
    passwordShow: "Parolni ko'rsatish",
    passwordHide: 'Parolni yashirish',
  },
  ru: {
    brandName: 'АНАЛИЗ РЫНКА',
    sidebarTitle: 'Кредитные продукты',
    sidebarFooter: 'SQB · Отдел стратегии',
    poweredByLabel: 'При поддержке SQB AI',
    adminSectionLabel: 'Управление',
    adminUsersNavLabel: 'Пользователи',
    pageSubtitle: 'Текущее состояние условий банков-конкурентов',
    tableRank: '#',
    tableBank: 'Банк',
    tableProduct: 'Продукт',
    tableRate: 'Ставка',
    tableTerm: 'Срок',
    tableDownPayment: 'Первоначальный взнос',
    tableGracePeriod: 'Льготный период',
    tableAmount: 'Сумма кредита',
    tableSpecialTerms: 'Особые условия',
    tablePaymentMethod: 'Способ оплаты',
    yes: 'Есть',
    no: 'Нет',
    dash: '-',
    defaultPaymentMethod: 'Аннуитет, Дифференцированный',
    emptyState: 'Для этой категории пока нет данных - страницы банков ожидают проверки.',
    houseFlag: 'Мы',
    pulseEyebrow: 'Пульс рынка · Лучшее предложение',
    pulseUpdated: 'Обновлено',
    pulseRate: 'Ставка',
    pulseTerm: 'Срок',
    pulseAmount: 'Сумма кредита',
    pulseCollateral: 'Залог',
    pulsePaymentMethod: 'Способ оплаты',
    pulseAiBadge: 'AI',
    pulseAiLabel: 'Совет от ИИ',
    pulseAiLoading: 'Анализируется...',
    unavailableReasonUnavailable: 'Продукт недоступен',
    unavailableReasonPaused: 'Временно приостановлено',
    langToggleLabel: 'Язык',
    langOptionUz: 'Узбекский',
    langOptionRu: 'Русский',
    monthUnit: 'мес.',
    loadingLabel: 'Загрузка',
    exportButton: 'Скачать в Excel',
    exportCurrentPage: 'Скачать текущую страницу',
    exportAllCategories: 'Скачать все категории',
    exportFailed: 'Не удалось скачать. Попробуйте ещё раз чуть позже.',
    refreshButton: 'Обновить данные',
    refreshConfirmTitle: 'Повторный сбор данных',
    refreshConfirmBody:
      'Данные всех банков будут собраны заново. Этот процесс займёт несколько минут - таблицы останутся в текущем виде, пока новые данные не будут готовы. Продолжить?',
    refreshConfirmYes: 'Да, начать',
    refreshConfirmCancel: 'Отмена',
    refreshStarted:
      'Обновление началось. Все банки проверяются заново - пожалуйста, обновите страницу примерно через 5 минут.',
    refreshAlreadyRunning: 'Обновление уже выполняется - пожалуйста, подождите немного.',
    refreshFailed: 'Не удалось запустить обновление. Попробуйте ещё раз чуть позже.',
    closeLabel: 'Закрыть',
    logoutButton: 'Выйти',
    authTitle: 'Вход в систему',
    authSubtitle: 'Введите логин и пароль, чтобы продолжить',
    authUsernameLabel: 'Логин',
    authPasswordLabel: 'Пароль',
    authSubmitButton: 'Войти',
    authError: 'Неверный логин или пароль',
    authRateLimited: 'Слишком много попыток. Пожалуйста, попробуйте снова через несколько минут.',
    authTagline: 'Анализ банков-конкурентов - в одном окне',
    adminPanelTitle: 'Управление пользователями',
    adminPanelSubtitle: 'Список пользователей с доступом к системе',
    adminAddUserButton: 'Добавить пользователя',
    adminUsernameLabel: 'Логин',
    adminRoleLabel: 'Роль',
    adminColCreatedAt: 'Дата добавления',
    adminEditButton: 'Редактировать',
    adminEditUserTitle: 'Редактирование пользователя',
    adminAddUserTitle: 'Новый пользователь',
    adminPasswordLabel: 'Пароль',
    adminPasswordLabelOptional: 'Новый пароль (необязательно)',
    adminPasswordPlaceholder: 'Оставьте пустым, чтобы не менять',
    adminPasswordHint: 'Минимум 8 символов',
    adminRoleUser: 'Пользователь',
    adminRoleAdmin: 'Администратор',
    adminSaveButton: 'Сохранить',
    adminUsernameTaken: 'Этот логин уже занят',
    adminSelfDemote: 'Вы не можете изменить свою роль',
    adminPasswordTooShort: 'Пароль должен содержать не менее 8 символов',
    adminSaveFailed: 'Не удалось сохранить. Попробуйте ещё раз.',
    adminLoadFailed: 'Не удалось загрузить пользователей',
    scrapeStatusTitle: 'Статус сбора данных',
    scrapeStatusSubtitle: 'Последняя попытка сбора данных по каждому банку',
    scrapeStatusLoadFailed: 'Не удалось загрузить статус сбора данных',
    scrapeColStatus: 'Статус',
    scrapeColLastRun: 'Последняя попытка',
    scrapeColProductsFound: 'Найдено продуктов',
    scrapeStatusSuccess: 'Успешно',
    scrapeStatusFailed: 'Ошибка',
    scrapeStatusRunning: 'Выполняется',
    scrapeStatusNoProducts: 'Требует внимания (0 продуктов)',
    scrapeStatusNeverRun: 'Ни разу не запускался',
    passwordShow: 'Показать пароль',
    passwordHide: 'Скрыть пароль',
  },
} as const satisfies Record<Lang, Record<string, string>>

export type TranslationKey = keyof (typeof UI_TEXT)['uz']

export function translate(lang: Lang, key: TranslationKey): string {
  return UI_TEXT[lang][key]
}
